var imgstr = ['img/0.jpg','img/1.png','img/2.png','img/3.png','img/4.png','img/5.png','img/6.png','img/7.png','img/8.png','img/9.png','img/10.png','img/11.png','img/12.png'];
...
var objwreckage = [], objgold = [], objtavern = [];
var gold = 0,incgold = 0;
var exp = 0;
var stage = 0;
var glev = 1, mlev = 2;
var actions_curr = 0, actions_max = 4;
...
var visitbonus = 0, wreckageindex = 0;
...
function check_wreckage(i)
{
	...
	if ( (x2+imgx[2]>x1)&&(x1+imgx[1]>x2)&&(y2+imgy[2]>y1)&&(y1+imgy[1]>y2) )
	{
		visittype = 1;
		/* это из второй части
		wrforce = 100+Math.floor( 100*Math.random() );
		while (wrforce>0)
		{
			do
			{
			...
			d = 3*army[0]+3*army[1]+3*army[2]+army[3];
			wrforce-=d;
			army[r]--;
		}
	...
	if ((e.clientX>=1060+75)&&(e.clientX<1060+150))
	{
		...
		if ((unum>=0)&&(unum<=3))
		{
			...
			if (enemyarmy[battempunit]>0)
			{
				...
				var enemychoose = Math.floor(4*Math.random());
