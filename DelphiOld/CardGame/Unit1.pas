...
function can_cthrow(co:colod;var reta:array of integer):boolean;
var res:boolean;
i:integer;
begin
res:=false;
...
begin
if s=-1 then result:='  '//nothing
else if s=0 then result:='Сброс'
else if s=1 then result:='Отбить'
else if s=2 then result:='Забрать'
else if s=3 then result:='Первый ход'
else result:='  ';
end;
...
//obreratnoe movanie,from colod from(not) last to colod to
...
begin
while (ct.size<count)and(ct.size>0) do
        cmove(cf,ct,0);
end;
//outinf
...
begin
Dg.ColCount:=co.size;
Dg.Width:=Dg.DefaultColWidth*co.size+9;//??
for i:=0 to co.size do Dg.Repaint;
end;
...
if eop=1 then
        ...
        if stat=0 then//sbros
                ...
                out_colod(DrawGrid1,c1);
                out_colod(DrawGrid2,c2);
                out_colod(DrawGrid3,ci1);
                out_colod(DrawGrid4,ci2);
                end;
        end;
end;
//SAVELOAD
        ...
        if eop=1 then SpeedButton5Click(SpeedButton5)
                ...
                if hod=0 then//fisrt hod
                        ...
                        if currig=1 then
                                ...
                                if can_cthrow(c1,ret)=true then
                                        begin
                                        SpeedButton1Click(SpeedButton1);
                                        for i:=0 to c1.size-1 do if reta[i]=1 then
                                                begin
                                                DrawGrid1.Col:=i;
                                                DrawGrid1DblClick(DrawGrid1);
                                                end;
                                ...
                                if can_cbeat(c1.a[c1.size-1],cg,ret)=true then
                                        begin
                                ...
                                begin
                                if can_cthrow(c2,ret)=true then
                                        begin
                                        SpeedButton1Click(SpeedButton1);
                                        for i:=0 to c2.size-1 do if reta[i]=1 then
                                                begin
                                                DrawGrid2.Col:=i;
                                                DrawGrid2DblClick(DrawGrid2);
                                                end;
                                ...
                                if can_cbeat(c2.a[c2.size-1],cg,ret)=true then
                                        begin
                                        SpeedButton2Click(SpeedButton2);
                                        DrawGrid2.Col:=ret;
                                        end else//sbros kozyrey
