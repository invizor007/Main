...
if r=0 then
        showmessage('Слишком много кладов. Начните новую игру');
end;
...
if (tip=0) and (res=0) then
        begin
        gold:=gold-opt[5];
        showmessage('1.Вы тратите '+inttostr(opt[5])+' золота');
        end;
if (tip=0) and (res>0) then
        begin
        klco:=klco-a[x1,x2].a;
        a[x1,x2].a:=0;
        gold:=gold-opt[5];
        showmessage('2.Вы тратите '+inttostr(opt[5])+' золота');
        errco:=errco+1;
        end;
if (tip=1) and (res=0) then
        begin
        gold:=gold-opt[5]-opt[4];
        showmessage('3.Вы теряете '+inttostr(opt[4]+opt[5])+' золота');
        errco:=errco+1;
        end;
if (tip=1) and (res>0) then
        begin
        ...
        klco:=klco-a[x1,x2].a;
        a[x1,x2].a:=0;
        showmessage('4.Вы нашли клад и получаете награду '+inttostr(res*100)+' золота');
        end;
...
if klco=0 then
        begin
        isendgame:=1;
        showmessage('Вы заработали '+inttostr(gold)+' очков');
        end;
if (opt[3]=1) and (errco>opt[4]) then
        begin
        isendgame:=-1;
        showmessage('Вы проиграли. Лимит ошибок');
        end;
if gold<0 then
        begin
        isendgame:=-1;
        showmessage('У вас нет денег на раскопки. Вы проиграли');
        end;
end;

procedure TForm1.OutInfo;
begin
La11.Caption:='Золото '+inttostr(gold);
La12.Caption:='Осталось '+inttostr(klco) +' кладов';
La13.Caption:='Ошибок '+inttostr(errco);
La14.Caption:='Ход номер '+inttostr(hodnum);
end;
...
if hodnum=0 then
        begin
        showmessage('Первый ход производится только разведка');
        SbT1.Down:=true;
        end;
end;
